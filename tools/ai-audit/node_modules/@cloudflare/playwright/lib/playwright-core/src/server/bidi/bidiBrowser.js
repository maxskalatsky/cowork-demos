...
import { Session, Network as Network$1, BrowsingContext, Script, Emulation } from './third_party/bidiProtocolCore.js';
...
class BidiBrowser extends Browser {
  ...
  static async connect(parent, transport, options) {
    ...
    await browser._browserSession.send("network.addDataCollector", {
      dataTypes: [Network$1.DataType.Response],
  ...
  async doUpdateRequestInterception() {
    if (this.requestInterceptors.length > 0 && !this._interceptId) {
      const { intercept } = await this._browser._browserSession.send("network.addIntercept", {
        phases: [Network$1.InterceptPhase.BeforeRequestSent],
...
export { BidiBrowser, BidiBrowserContext, Network, getScreenOrientation };
