...
import { formatObject, formatObjectOrVoid, escapeWithQuotes } from '../../utils/isomorphic/stringUtils.js';
...
class JavaScriptLanguageGenerator {
  ...
  _generateActionCall(subject, actionInContext) {
    ...
    switch (action.name) {
      ...
      case "hover":
        return `await ${subject}.${this._asLocator(action.selector)}.hover(${formatOptions({ position: action.position })});`;
      case "check":
