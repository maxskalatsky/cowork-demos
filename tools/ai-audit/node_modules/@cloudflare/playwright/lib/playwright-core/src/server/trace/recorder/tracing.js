...
class Tracing extends SdkObject {
  ...
  start(options) {
    ...
    this._state = {
      ...
      traceName,
      tracesDir,
      traceFile: path__default.join(tracesDir, traceName + ".trace"),
      networkFile: path__default.join(tracesDir, traceName + ".network"),
      resourcesDir: path__default.join(tracesDir, "resources"),
      chunkOrdinal: 0,
  ...
  async startChunk(progress, options = {}) {
    ...
    if (!preserveNetworkResources)
      this._fs.writeFile(this._state.networkFile, "");
    this._fs.mkdir(path__default.dirname(this._state.traceFile));
    const event = {
      ...this._contextCreatedEvent,
      title: options.title,
      wallTime: Date.now(),
  ...
  _allocateNewTraceFile(state) {
    ...
    state.chunkOrdinal++;
    state.traceFile = path__default.join(state.tracesDir, `${state.traceName}${suffix}.trace`);
  }
  _changeTraceName(state, name, preserveNetworkResources) {
    state.traceName = name;
    state.chunkOrdinal = 0;
    this._allocateNewTraceFile(state);
    const newNetworkFile = path__default.join(state.tracesDir, name + ".network");
    if (preserveNetworkResources)
  ...
  _createTracesDirIfNeeded() {
    ...
    this._tracesTmpDir = fs.mkdtempSync(path__default.join(os.tmpdir(), "playwright-tracing-"));
  ...
  async stopChunk(progress, params) {
    ...
    this.flushHarEntries();
    const newNetworkFile = path__default.join(this._state.tracesDir, this._state.traceName + `-pwnetcopy-${this._state.chunkOrdinal}.network`);
    const entries = [];
    ...
    for (const sha1 of /* @__PURE__ */ new Set([...this._state.traceSha1s, ...this._state.networkSha1s]))
      entries.push({ name: path__default.join("resources", sha1), value: path__default.join(this._state.resourcesDir, sha1) });
  ...
  _appendResource(sha1, buffer) {
    ...
    const resourcePath = path__default.join(this._state.resourcesDir, sha1);
    this._fs.writeFile(
      resourcePath,
      buffer,
      true
