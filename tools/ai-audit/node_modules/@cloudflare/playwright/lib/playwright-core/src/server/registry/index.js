...
import { hostPlatform, isOfficiallySupportedPlatform, shortPlatform } from '../utils/hostPlatform.js';
...
const PACKAGE_PATH = path__default.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..");
const BIN_PATH = path__default.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..", "bin");
...
const registryDirectory = (() => {
  ...
  if (envDefined === "0") {
    result = path__default.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..", ".local-browsers");
  } else if (envDefined) {
    result = envDefined;
  } else {
    let cacheDirectory;
    if (process.platform === "linux")
      cacheDirectory = process.env.XDG_CACHE_HOME || path__default.join(os.homedir(), ".cache");
    else if (process.platform === "darwin")
      cacheDirectory = path__default.join(os.homedir(), "Library", "Caches");
    else if (process.platform === "win32")
      cacheDirectory = process.env.LOCALAPPDATA || path__default.join(os.homedir(), "AppData", "Local");
    else
      throw new Error("Unsupported platform: " + process.platform);
    result = path__default.join(cacheDirectory, "ms-playwright");
  }
  if (!path__default.isAbsolute(result)) {
    result = path__default.resolve(getFromENV("INIT_CWD") || process.cwd(), result);
  }
  return result;
})();
function isBrowserDirectory(browserDirectory) {
  const baseName = path__default.basename(browserDirectory);
...
function readDescriptors(browsersJSON) {
  return browsersJSON["browsers"].map((obj) => {
    ...
    const descriptor = {
      ...
      dir: path__default.join(registryDirectory, browserDirectoryPrefix.replace(/-/g, "_") + "-" + revision)
    };
    return descriptor;
  });
}
...
class Registry {
  constructor(browsersJSON) {
    ...
    const findExecutablePath = (dir, name) => {
      ...
      return tokens ? path__default.join(dir, ...tokens) : void 0;
    };
...
${wrapInASCIIBox(prettyMessage, 1)}`);
    ...
    const webkitLinuxLddDirectories = [
      path__default.join("minibrowser-gtk"),
      path__default.join("minibrowser-gtk", "bin"),
      path__default.join("minibrowser-gtk", "lib"),
      path__default.join("minibrowser-gtk", "sys", "lib"),
      path__default.join("minibrowser-wpe"),
      path__default.join("minibrowser-wpe", "bin"),
      path__default.join("minibrowser-wpe", "lib"),
      path__default.join("minibrowser-wpe", "sys", "lib")
    ];
    this._executables.push({
      name: "webkit",
      browserName: "webkit",
    ...
    this._executables.push({
      ...
      _install: async () => {
        ...
        const script = path__default.join(BIN_PATH, "install_webkit_wsl.ps1");
  ...
  _createChromiumChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      for (const prefix of prefixes) {
        const executablePath2 = path__default.join(prefix, suffix);
        if (canAccessFile(executablePath2))
          return executablePath2;
      }
      if (!shouldThrow)
        return void 0;
      const location = prefixes.length ? ` at ${path__default.join(prefixes[0], suffix)}` : ``;
...
Run "${buildPlaywrightCLICommand(sdkLanguage, "install " + name)}"` : "";
  ...
  ${prefixes.map((p) => path__default.join(p, suffix)).join("\n  ")}`}`);
      return void 0;
    };
    return {
      name,
  ...
  async _validateHostRequirements(sdkLanguage, browserDirectory, linuxLddDirectories, dlOpenLibraries, windowsExeAndDllDirectories) {
    if (os.platform() === "linux")
      return await validateDependenciesLinux(sdkLanguage, linuxLddDirectories.map((d) => path__default.join(browserDirectory, d)), dlOpenLibraries);
    if (os.platform() === "win32" && os.arch() === "x64")
      return await validateDependenciesWindows(sdkLanguage, windowsExeAndDllDirectories.map((d) => path__default.join(browserDirectory, d)));
  }
  ...
  async install(executablesToInstall, options) {
    ...
    const lockfilePath = path__default.join(registryDirectory, "__dirlock");
    const linksDir = path__default.join(registryDirectory, ".links");
    let releaseLock;
    try {
      ...
      await fs.promises.writeFile(path__default.join(linksDir, calculateSha1(PACKAGE_PATH)), PACKAGE_PATH);
  ...
  async uninstall(all) {
    ...
    if (all) {
      ...
      for (const link of links)
        await fs.promises.unlink(path__default.join(linksDir, link));
    } else {
      await fs.promises.unlink(path__default.join(linksDir, calculateSha1(PACKAGE_PATH))).catch(() => {
      });
    }
    await this._validateInstallationCache(linksDir);
    return {
  ...
  async _validateHostRequirementsForExecutableIfNeeded(executable, sdkLanguage) {
    ...
    if (!executable.directory)
      return;
    const markerFile = path__default.join(executable.directory, "DEPENDENCIES_VALIDATED");
  ...
  _downloadURLs(descriptor) {
    ...
    if (typeof downloadPathTemplate === "function") {
      ...
      downloadPath = require$$0$2.format(downloadPathTemplate, descriptor.revision);
...
${e.stack}`);
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    if (isPowerShell) {
      const args = [
        ...
        "Bypass",
        "-File",
        path__default.join(BIN_PATH, scriptName),
        ...scriptArgs
      ];
      ...
      const { command, args, elevatedPermissions } = await transformCommandsForRoot([`bash "${path__default.join(BIN_PATH, scriptName)}" ${scriptArgs.join("")}`]);
  ...
  async _traverseBrowserInstallations(linksDir) {
    ...
    for (const fileName of await fs.promises.readdir(linksDir)) {
      const linkPath = path__default.join(linksDir, fileName);
      let linkTarget = "";
      try {
        linkTarget = (await fs.promises.readFile(linkPath)).toString();
        const browsersJSON = commonjsRequire(path__default.join(linkTarget, "browsers.json"));
        const descriptors = readDescriptors(browsersJSON);
        for (const browserName of allDownloadableDirectoriesThatEverExisted) {
  ...
  async _deleteStaleBrowsers(browserList) {
    ...
    let downloadedBrowsers = (await fs.promises.readdir(registryDirectory)).map((file) => path__default.join(registryDirectory, file));
...
function browserDirectoryToMarkerFilePath(browserDirectory) {
  return path__default.join(browserDirectory, "INSTALLATION_COMPLETE");
}
...
export { Registry, browserDirectoryToMarkerFilePath, buildPlaywrightCLICommand, findChromiumChannelBestEffort, registry, registryDirectory };
