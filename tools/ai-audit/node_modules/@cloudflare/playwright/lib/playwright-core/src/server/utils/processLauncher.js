import { spawn, spawnSync } from '../../../../mocks/childProcess.js';
...
async function launchProcess(options) {
  ...
  ({
    ...
    cwd: options.cwd,
    shell: options.shell});
  const spawnedProcess = spawn(options.command, options.args || []);
  const cleanup = async () => {
  ...
  const stdout = createInterface({ input: spawnedProcess.stdout });
  stdout.on("line", (data) => {
    options.log(`[pid=${spawnedProcess.pid}][out] ` + data);
  });
  const stderr = createInterface({ input: spawnedProcess.stderr });
  stderr.on("line", (data) => {
